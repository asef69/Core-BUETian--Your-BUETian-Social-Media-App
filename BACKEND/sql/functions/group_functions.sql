...
SELECT g.id AS group_id,
    ...
    g.description,
    g.cover_image,
    -- Total accepted members in the group
    (
        SELECT COUNT(*)
    ...
    g.is_private,
    -- Count of members who are also followed by the current user
    (
        SELECT COUNT(*)
        FROM group_members gm
        WHERE gm.group_id = g.id
...
END;
$$ LANGUAGE plpgsql;
-- Full details of a group with membership status for the viewer
